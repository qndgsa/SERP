...
cursor = db.cursor()
cursor.execute("SELECT query, entry_file, sequence FROM config_data WHERE config_id = (%s)", (config_id,))
config_data = cursor.fetchall()


# data config
query = config_data[0][0]

entry_file = config_data[0][1]

sequence = config_data[0][2]  # the sequence determine the display order of entries by category
    ...
    if exception_pos != "E" and E_counter != len(effective):
        ...
        with template.div(klass="searchresult"):
            with template.a():
                template(
                    entry['URL'])
            with template.a(href=entry['URL'], target="_blank", style="text-decoration: none;", onclick="url(" + str(position_counter) + ");"):
                with template.h2():
                    template(
                        entry['title'])
            with template.p():
            ...
            with template.p(id="searchresultsnumber"):
                ...
                for item in sequence:
                    if item == 'E':
                        if E_counter >= len(effective):
                            E_counter, I_counter, U_counter = entry_exception(E_counter, I_counter, U_counter, "E", position_counter)
                        else:
                            entry = effective[E_counter]
                            with template.div(klass="searchresult"):
                                with template.p():
                        ...
                        if I_counter >= len(ineffective):
                            E_counter, I_counter, U_counter = entry_exception(E_counter, I_counter, U_counter, "I", position_counter)
                        else:
                            entry = ineffective[I_counter]
                            with template.div(klass="searchresult"):
                                with template.p():
                        ...
                        if U_counter >= len(unknown):
                            E_counter, I_counter, U_counter = entry_exception(E_counter, I_counter, U_counter, "U", position_counter)
                        else:
                            entry = unknown[U_counter]
                            with template.div(klass="searchresult"):
                                with template.p():
                        ...
                        if A['img_ad'] :
                            with template.card(style="padding:10px;"):
                                template(
                                    "<b style=\"color:black;display:inline-block;\">Ads · &nbsp; </b>" + "<p style=\"display:inline-block;\">"+" "+data['query']+"</p>")
                                            ...
                                            with template.a(href=A['URL'],target="_blank" , onclick="ad();"):
                                                with template.div(klass="img-container",style="padding:4px;background-color:#F9F9F9"):
                                                    with template.div(klass="img-container-sizer",style="display:inline-block;text-align:center;height:114px;width:114px"):
                                                        ...
                                                        template(
                                                            "<img style=\"border:none;margin:0;vertical-align:middle;border-radius:0%\" height=\"114\" src=\""+A['img_link']+"\" width=\"114\">")
                                    with template.div(klass="ad-title",style="width:135px; height: 100px;"):
                                        with template.div(klass="title-container"):
                                            with template.div(klass="title", style="height:50px"):
                                                with template.a(href=A['URL'],target="_blank",style="text-decoration: none;", onclick="ad();"):
                ...
                if len(sequence) < (len(effective) + len(ineffective) + len(unknown)+1) and fill == True:
                    ...
                    for i in range(0,fill_up):
                        E_counter, I_counter, U_counter = entry_exception(E_counter, I_counter, U_counter, "", position_counter)
                        position_counter += 1
